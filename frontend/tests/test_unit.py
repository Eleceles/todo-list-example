...
class TestCreate(TestBase):
    def test_create_task(self):
        response = self.client.post(url_for('create_task'), data={"description": "Testing create funtionality"}, follow_redirects=True)
        self.assertIn(b"Testing create funtionality", response.data)
    
class Testupdate(TestBase):
    def test_update_task(self):
        response = self.client.post(url_for('update_task', id=1), data={"description": "Testing update funtionality"}, follow_redirects=True)
